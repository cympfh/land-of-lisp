...
;; find not connected nodes (islands, 孤島)

(defun direct-edges (node edge-list)
  (remove-if-not (lambda (edge) (equal (car edge) node))
                 edge-list))
    ...
    (labels
      ((find-island (nodes)
        (let* ((connected (get-connected (car nodes)
                                         edge-list))
               (uconnected (set-difference nodes
                                           connected)))
          (push connected islands)
          (when uconnected
            (find-island uconnected)))))
      (find-island nodes))
    islands))
...
(defun make-city-edges ()
  (let* ((nodes (loop for i from 1 to *node-num*
                      collect i))
         (edge-list (connect-all-islands nodes
                                         (make-edge-list)))
         (cops (remove-if-not
...
(defun new-game ()
  ...
  (setf *congestion-city-nodes*
        (make-city-nodes *congestion-city-edges*))
...
(defun draw-city ()
  (ugraph->png "city" *congestion-city-nodes*
